...
class ModelOpenbayOpenbay extends Model {
	...
	public function updateV2Test() {
		...
		$web_root = preg_replace('/core\/$/', '', DIR_CORE);
		...
		// create a tmp folder
		if (!is_dir($web_root . '/core/download/tmp')) {
			try {
				mkdir($web_root . '/core/download/tmp');
		...
		// create tmp file
		try {
			$tmp_file = fopen($web_root . '/core/download/tmp/test_file.php', 'w+');
		} catch(ErrorException $ex) {
			$this->error[] = $ex->getMessage();
		}
		...
		// remove tmp file
		try {
			unlink($web_root . '/core/download/tmp/test_file.php');
		} catch(ErrorException $ex) {
			$this->error[] = $ex->getMessage();
		}
		...
		// delete tmp folder
		try {
			rmdir($web_root . '/core/download/tmp');
		} catch(ErrorException $ex) {
			$this->error[] = $ex->getMessage();
		}
	...
	public function updateV2Download($beta = 0) {
		...
		$local_file = $web_root . 'core/download/openbaypro_update.zip';
	...
	public function updateV2Extract() {
		...
		try {
			...
			if ($zip->open($web_root . 'core/download/openbaypro_update.zip')) {
