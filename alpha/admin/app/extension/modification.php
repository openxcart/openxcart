 ...
 *
 * https://github.com/opencart/opencart/wiki/Modification-core
 */
class AppExtensionModification extends App {
	private $error = array();
			...
			foreach ($xml as $xml) {
				...
				foreach ($files as $file) {
					...
					foreach ($files as $file) {
						...
						if (substr($file, 0, 6) == 'core') {
							$path = DIR_CORE . str_replace('../', '', substr($file, 7));
						}
						...
						if ($path) {
							...
							if ($files) {
								foreach ($files as $file) {
									...
									if (substr($file, 0, strlen(DIR_CORE)) == DIR_CORE) {
										$key = 'core/' . substr($file, strlen(DIR_CORE));
									}
