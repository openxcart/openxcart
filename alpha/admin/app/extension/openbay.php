<?php
class AppExtensionOpenbay extends App {
	private $error = array();
	...
	public function install() {
		...
		if (!$this->user->hasPermission('modify', 'extension/openbay')) {
			...
			require_once(DIR_APPLICATION . 'app/openbay/' . $this->request->get['extension'] . '.php');

			$class = 'AppOpenbay' . str_replace('_', '', $this->request->get['extension']);
	...
	public function index() {
		...
		foreach ($extensions as $key => $value) {
			if (!file_exists(DIR_APPLICATION . 'app/openbay/' . $value . '.php')) {
	...
	public function purge() {
		...
		$this->log->write('User is trying to wipe core data');
