<?php
class AppExtensionInstaller extends App {
	private $error = array();
				...
				if (file_exists($file)) {
					...
					if ($zip) {
						...
						while ($entry = zip_read($zip)) {
							...
							// Compare core files
							$file = DIR_CORE . substr($zip_name, 14);

							if (is_file($file) && substr($zip_name, 0, 14) == 'upload/core/') {
								$json['overwrite'][] = substr($zip_name, 7);
							}
						}
				...
				if ($login) {
					...
					if ($root) {
						foreach ($files as $file) {
							...
							if (substr($destination, 0, 6) == 'core') {
								$destination = basename(DIR_CORE) . substr($destination, 6);
							}
