<?php
class AppModuleAmazonPay extends App {
	public function index() {
		...
		if ($this->config->get('amazon_login_pay_status') && $this->config->get('amazon_pay_status') && !empty($_SERVER['HTTPS']) && !($this->config->get('amazon_login_pay_minimum_total') > 0 && $this->config->get('amazon_login_pay_minimum_total') > $this->cart->getTotal())) {
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/amazon_pay.html')) {
				return $this->load->view($this->config->get('config_template') . '/template/module/amazon_pay.html', $data);
			} else {
				return $this->load->view('default/template/module/amazon_pay.html', $data);
			}
		}
	}
