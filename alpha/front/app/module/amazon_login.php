<?php
class AppModuleAmazonLogin extends App {
	public function index() {
		...
		if ($this->config->get('amazon_login_pay_status') && $this->config->get('amazon_login_status') && !$this->customer->isLogged() && !empty($this->request->server['HTTPS'])) {
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/amazon_login.html')) {
				return $this->load->view($this->config->get('config_template') . '/template/module/amazon_login.html', $data);
			} else {
				return $this->load->view('default/template/module/amazon_login.html', $data);
			}
		}
	}
