<?php
class AppCheckoutVoucher extends App {
	public function index() {
		if ($this->config->get('voucher_status')) {
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/checkout/voucher.html')) {
				return $this->load->view($this->config->get('config_template') . '/template/checkout/voucher.html', $data);
			} else {
				return $this->load->view('default/template/checkout/voucher.html', $data);
			}
		}
	}
